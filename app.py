...
if "quick_lookup_current" not in st.session_state:
    st.session_state.quick_lookup_current = None  # 直近に検索した語(小文字キー)
...
def _display_gemini_error(exc: GeminiServiceError) -> None:
    """429(レート制限)は突き放さず、代替手段を案内する親切なメッセージにする。"""
...
def render_keyword_dict(kw: KeywordDictItem) -> None:
    """単語辞書1件分を、popover・サイドバー共通の見た目で表示する。"""
    ...
    if st.button("調べる", key="quick_lookup_button"):
        ...
        if not term:
            ...
            if cache_key in st.session_state.quick_lookup_cache:
                st.session_state.quick_lookup_current = cache_key

    current_key = st.session_state.quick_lookup_current
    if current_key and current_key in st.session_state.quick_lookup_cache:
        kw = normalize_keyword(st.session_state.quick_lookup_cache[current_key])
        with st.container(border=True):
            render_keyword_dict(kw)
            st.divider()
            if st.button(
                "🚀 この単語で30例文を生成して深掘り ➔",
                key="quick_lookup_derive",
            ...
            for i, ex in enumerate(examples):
                with st.container():
                    ...
                    if st.button("この内容から派生学習 ➔（例文全体）", key=f"{prefix}_{i}"):
                        derive(
                            ex.en, [normalize_keyword(kw).term for kw in ex.key_terms], prefix, i
                        )
                    st.divider()
